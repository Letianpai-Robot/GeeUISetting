...
class SelectStartFragment : BaseSecondFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view?.findViewById<TextView>(R.id.title)
            ?.setText(resources.getString(R.string.time_starttime_desc))
        save?.setOnClickListener {
            Log.i("TAG", "onViewCreated: ${hour}:${min} ")
            if (sleepConfig == null) {
                return@setOnClickListener
            }
            sleepConfig?.startTime = "$hour:$min"
            updateData()
        }
        getData()
    }
    ...
    private fun updateUI() {
        activity?.runOnUiThread {
            var time = sleepConfig?.startTime?.split(":")
            hour = time?.get(0) ?: "12"
            min = time?.get(1) ?: "30"
            activity?.runOnUiThread {
